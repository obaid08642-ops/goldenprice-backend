...
// Optional: GoldAPI / GoldPriceZ (used only if keys exist and everything else failed)
...
async function srcGoldPriceZ(metal="XAU", currency="USD", tag="goldpricez"){
  ...
  // convert gram -> ounce troy (31.1034768 g/oz)
  return v * 31.1034768;
}
...
async function getMetalUSD(metal){
  ...
  // A) Primary round (TwelveData then Ninjas)
  const A = [];
  ...
  // C) Limited/Backup (MarketStack, GoldAPI/GoldPriceZ if available)
  const C = [];
  if (m.c && MARKETSTACK_KEY) C.push(()=>srcMarketStack(m.c, "ms:"+m.c));
  if (GOLDAPI_KEY && m.c) C.push(()=>srcGoldAPI(m.c, "USD", "goldapi"));
  if (GOLDPRICEZ_KEY && ["XAU","XAG","XPT","XPD"].includes(m.c)) C.push(()=>srcGoldPriceZ(m.c, "USD", "gpz"));
  for (const fn of C){
    try {
      const v = await fn();
