...
const failures = new Map();     // name -> count
const successes = new Map();    // name -> count
const sourceOK = (name)=> (backoffUntil.get(name)||0) < now();
const punish = (name, ms=180000)=> backoffUntil.set(name, now()+ms);
const markFail = (name)=> failures.set(name, 1+(failures.get(name)||0));
const markOK   = (name)=> successes.set(name, 1+(successes.get(name)||0));

async function tryJson(url, opts={}, retries=1, tag=""){
  let lastErr;
...
async function tryText(url, opts={}, retries=1, tag=""){
  let lastErr;
  for(let i=0;i<=retries;i++){
    try{
      const r = await fetch(url, opts);
...
const METALS = {
  gold:      { unit:"oz", td:"XAU/USD", ninjas:"xau", y:"XAUUSD=X", c:"XAU" },
  silver:    { unit:"oz", td:"XAG/USD", ninjas:"xag", y:"XAGUSD=X", c:"XAG" },
  platinum:  { unit:"oz", td:"XPT/USD", ninjas:"xpt", y:"XPTUSD=X", c:"XPT" },
  palladium: { unit:"oz", td:"XPD/USD", ninjas:"xpd", y:"XPDUSD=X", c:"XPD" },

  copper:    { unit:"lb", td:null, ninjas:"copper",   y:"HG=F",  c:"HG" },
...
const TTL = { metals: 60_000, fx: 60_000, crypto: 10_000 };
...
async function srcTwelve(symbol, tag){
  ...
  const j = await tryJson(url, {}, 1, tag);
...
async function scrapeXRatesGoldUSD(tag){
  ...
  const html = await tryText("https://www.x-rates.com/commodity/gold", {}, 1, tag);
...
async function getMetalUSD(metal){
  ...
  // A) Primary (TwelveData + Ninjas + GoldPriceZ للثمين)
  const A = [];
  if (m.td && TWELVEDATA_KEY) A.push(()=>srcTwelve(m.td, "td:"+m.td));
  if (m.ninjas && API_NINJAS_KEY) A.push(()=>srcNinjas(m.ninjas, "ninjas:"+m.ninjas));
  if (m.c && GOLDPRICEZ_KEY && ["XAU","XAG","XPT","XPD"].includes(m.c))
    A.push(()=>srcGoldPriceZ(m.c, "USD", "gpz-primary"));
  ...
  if (GOLDPRICEZ_KEY && ["XAU","XAG","XPT","XPD"].includes(m.c)) C.push(()=>srcGoldPriceZ(m.c, "USD", "gpz-backup"));
  for (const fn of C){
    try {
      const v = await fn();
...
async function getFX(from="USD", to="EGP"){
  ...
  try{
    if (TWELVEDATA_KEY){
      const j = await tryJson(`https://api.twelvedata.com/price?symbol=${from}/${to}&apikey=${TWELVEDATA_KEY}`, {}, 1, "fx:td");
      const v = Number(j?.price);
...
async function getCryptoUSDT(sym="BTC"){
  ...
  try{
    const j = await tryJson(`https://api.coingecko.com/api/v3/simple/price?ids=${sym.toLowerCase()}&vs_currencies=usd`, {}, 1, "cg");
  ...
  try{
    ...
    const j = await tryJson(`https://api.kraken.com/0/public/Ticker?pair=${kr}`, {}, 1, "kraken");
...
app.get("/api/status", (req,res)=>{
  const nowTs = now();
  const status = {};
  for (const [name, ts] of backoffUntil.entries()){
    status[name] = { backoffUntil: ts, msLeft: Math.max(0, ts-nowTs) };
  }
  res.json({
    ok:true,
...
app.get("/", (req,res)=>{
  res.json({
    ...
    routes: [
      "/api/health",
      "/api/status",
      "/api/metals?list=gold,silver",
      "/api/fx?from=USD&to=EGP",
      "/api/crypto?list=BTC,ETH"
    ]
  });
});

app.listen(PORT, ()=> console.log("✅ GoldenPrice Backend running on port "+PORT));
